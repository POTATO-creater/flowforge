...
/** 界面模式：basic = 小白模式（隐藏高级字段），pro = 大佬模式（全部可见） */
...
export interface AgentConfig {
  ...
  system: string;
  tools: string; // 工具定义 JSON（OpenAI tools 数组）
...
export interface MergeConfig {
  ...
  template: string; // template 模式：用 {{节点ID.字段}} 引用
}
...
export type FieldType = 'text' | 'textarea' | 'number' | 'select' | 'code';
